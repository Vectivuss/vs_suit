...
local function _SharedAbilityInfo( i, x, y, w, h )
    ...
    if pos[i].x and pos[i].y then
        ...
        do // stats info
            local _y = y+h*.062
            local _, yY = draw.SimpleText( VectivusSuits.Language["Suit_Ability_Start"] .. (t.start or 0) .. "s", "vs.suits.ui.22", x+h*.008, _y, VectivusSuits.Colors["white"], 0, 3 )
            _y = _y + yY
            local _, yY = draw.SimpleText( VectivusSuits.Language["Suit_Ability_Cooldown"] .. (t.cooldown or 0) .. "s", "vs.suits.ui.22", x+h*.008, _y, VectivusSuits.Colors["white"], 0, 3 )
            _y = _y + yY +10
